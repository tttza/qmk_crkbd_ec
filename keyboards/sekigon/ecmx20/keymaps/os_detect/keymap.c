...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed == true) {
        if (keycode == KC_FN1) {
            switch (get_usb_host_os_type()) {
                case OS_TYPE_WIN:
                    send_string("OS Type is Windows\n");
                    break;
                case OS_TYPE_MAC:
                    send_string("OS Type is Mac\n");
                    break;
                case OS_TYPE_LINUX:
                    send_string("OS Type is Linux\n");
                    break;
                default:
                    send_string("OS Type is Unknown\n");
                    break;
            }
...
void matrix_scan_user(void) { host_os_eeconfig_update(get_usb_host_os_type()); }
